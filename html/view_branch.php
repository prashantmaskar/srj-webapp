...
{
 $sql_query ="DELETE FROM branch WHERE branch_id=".$_GET['delete_id'];
 echo  "dfdfdf" .$sql_query;
 mysqli_query($connection,$sql_query);
 header("Location: http://srjcreations.com/admin/html/View_branch.php");
} 
 ?>
...
{
 if(confirm('Sure To Remove This Branch ?'))
 {
  window.location.href='http://localhost/SRJ/html/View_branch.php?delete_id='+id;
 }
}
</script>
</head>
      ...
      <table ui-jp="dataTable" class="table table-striped b-t b-b">
        <thead>
          <tr>
            ...
            <th  style="width:15%"> Branch Location</th>
            <th  style="width:13%">Branch Contact</th>
          ...
          <?php
           $branch_query = "select * from branch";
            $branch_result = mysqli_query($connection, $branch_query);
             ?>
             <?php
                                                        while ($b_info = mysqli_fetch_array($branch_result)) {
                                                            ?>
          <tr>
            ...
            <td><?php echo $b_info[4]; ?></td>
            <td>
             <a class="btn btn-icon btn-rounded btn-info" href="http://srjcreations.com/admin/html/edit_branch.php?id=<?php echo $b_info[0]; ?>">
