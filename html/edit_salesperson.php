...
include_once 'api/config/database.php';
 ...
 if (isset($_GET['id'])) {
            $query = "select * from admin_account where admin_id = ". $_GET['id']." AND admin_type =1";
            $result = mysqli_query($connection,$query);
            $sperson_rs = mysqli_fetch_array($result, MYSQLI_ASSOC);
...
<div class="padding">
  ...
  <div class="row">
    <div class="col-sm-12">
      <form ui-jp="parsley" id="editsperson" method="post">
        <div class="box">
          <div class="box-header">
            <h2>Add Sales Pesron</h2>
          </div>
          <div class="box-body">
            <p class="text-muted">Please fill the information to continue</p>
            <div class="row m-b">
            <div class="col-md-6">
              <input type="hidden" name="s_id" value="<?php echo $sperson_rs['admin_id']; ?>">
              <label>Username</label>
              <input type="text" name="s_user" value="<?php echo $sperson_rs['name']; ?>" class="form-control">                        
            </div>
            <div class="col-md-6">
              <label>Email</label>
              <input type="email" name="s_email" value="<?php echo $sperson_rs['email']; ?>" class="form-control" >                        
            </div>
          </div>
            <div class="row m-b">
              <div class="col-sm-6">
                <label>Enter password</label>
                <input type="password" name="s_pass" class="form-control"  id="pwd">   
              </div>
              <div class="col-sm-6">
                <label>Confirm password</label>
                <input type="password" name="s_cpass" class="form-control" >      
              </div>   
            </div>
            <div class="row m-b">
            <div class="col-sm-6">
              <label>Phone</label>
              <input type="number" name="s_contact" value="<?php echo $sperson_rs['mobile_no']; ?>" class="form-control" placeholder="XXX XXXX XXX" >
            </div>
            <div class="col-sm-6">
              <label>Region</label>
            <select  class="form-control c-select " name="s_region">
             <option value="">Please choose Region</option>
             <?php 
                $bqry="SELECT * FROM `region`;";
                $brun=mysqli_query($connection,$bqry);
                   while( $region = mysqli_fetch_assoc($brun) ):
                     ?>
          ...
          <div class="dker p-a text-right">
            <input type="submit" class="btn info" value="update" name="submit">
          </div>
        </div>
      </form>
    </div>
