...
{
 $sql_query ="DELETE FROM product WHERE product_id=".$_GET['delete_id'];
 echo  "dfdfdf" .$sql_query;
 mysqli_query($connection,$sql_query);
 header("Location: http://srjcreations.com/admin/html/view_product.php");
} 
 ?>
...
function delete_id(id)
{
 ...
 {
  window.location.href='http://srjcreations.com/admin/html/view_product.php?delete_id='+id;
 }
}
</script>
</head>
      ...
      <table ui-jp="dataTable" class="table table-striped b-t b-b">
        <thead>
          <tr>
            ...
            <th  style="width:25%">Brand Id</th>
            <th  style="width:25%">Product Created On</th>
            <th  style="width:25%">Action</th>
          </tr>
        </thead>
        <tbody>
          <?php
          $path = "http://srjcreations.com/asset/";
           $brand_query = "SELECT p.* FROM ( SELECT product_id, max(product_created_on) AS product_created_on FROM product GROUP BY product_id ) AS mx JOIN product p ON mx.product_id = p.product_id AND mx.product_created_on = p.product_created_on ORDER BY product_id";
            $brand_result = mysqli_query($connection, $brand_query);
             ?>
             <?php
                                                        while ($b_info = mysqli_fetch_array($brand_result)) {
                                                            ?>
          <tr>
            <td><?php echo $b_info[0]; ?></td>
            <td><?php echo $b_info[2]; ?></td>
            <td><?php echo $b_info[1]; ?></td>
            <td><?php echo $b_info[3]; ?></td>
            
            <td>
              <a class="btn btn-icon btn-rounded btn-info" href="http://srjcreations.com/admin/html/edit_product.php?id=<?php echo $b_info[0]; ?>">
                <i class="fa fa-pencil"></i>
              </a>
              <button class="btn btn-icon btn-rounded btn-danger" onclick="delete_id(<?php echo $b_info[0]; ?>);">
