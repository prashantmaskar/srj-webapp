...
{
 $sql_query ="DELETE FROM event WHERE event_id=".$_GET['delete_id'];
 mysqli_query($connection,$sql_query);
 header("http://srjcreations.com/admin/html/view_event.php");
} 
 ?>
...
function delete_id(id)
{
 ...
 {
  window.location.href='http://srjcreations.com/admin/html/view_event.php?delete_id='+id;
 }
}
</script>
</head>
      ...
      <table ui-jp="dataTable" class="table table-striped b-t b-b">
        <thead>
          <tr>
            ...
            <th  style="width:25%">Event Place</th>
        ...
        <tbody>
          <?php
           $brand_query = "select * from event";
            $event_result = mysqli_query($connection, $brand_query);
             ?>
             <?php
                                                        while ($e_info = mysqli_fetch_array($event_result)) {
                                                            ?>
          <tr>
            <td><?php echo $e_info[0]; ?></td>
            <td><?php echo $e_info[1]; ?></td>
            <td><?php echo $e_info[2]; ?></td>
            <td><?php echo $e_info[5]; ?></td>
            <td>
             <a class="btn btn-icon btn-rounded btn-info" href="http://srjcreations.com/admin/html/edit_event.php?id=<?php echo $e_info[0]; ?>">
                <i class="fa fa-pencil"></i>
              </a>
              <button class="btn btn-icon btn-rounded btn-danger" onclick="delete_id(<?php echo $e_info[0]; ?>);">
